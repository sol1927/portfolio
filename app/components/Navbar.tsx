...
const Navbar = () => {
  ...
  return (
    <>
      <header
        id="navbar"
        className="fixed top-0 left-0 w-full z-50 p-4 sm:p-6 flex justify-between items-center backdrop-blur-xs bg-black/10 shadow"
      >
        <div className='w-16 sm:w-20 aspect-square rounded-full overflow-hidden shadow-[2px_10px_10px_2px] shadow-gray-700 hover:scale-110 hover:shadow-blue-500 transition-all duration-300'>
          <img
            src="/SA.png"
            alt="logo"
        ...
        <div className='flex items-center gap-4'>
          <button
            className='flex md:hidden justify-center items-center hover:cursor-pointer hover:scale-110 transition-all duration-300 bg-gray-300 rounded-full p-2 shadow-lg shadow-gray-600 hover:shadow-blue-500'
      ...
      <div
        className={`fixed inset-0 bg-black/70 z-40 transition-opacity duration-300 ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
        onClick={() => setMenuOpen(false)}
      ></div>

      <div
        className={`fixed top-0 right-0 h-screen w-64 md:hidden z-50 flex flex-col items-start gap-6 bg-blue-950 text-white p-4 shadow-lg shadow-black transform transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
